...
---Pandoc coerces wherever the manual documents a `Blocks` parameter: a plain
---array, a single `Block`, and a string are all accepted and converted.
---Verified against pandoc 3.11 -- `pandoc.Div("x")`, `pandoc.Div(pandoc.Para"x")`
---and `pandoc.Div{pandoc.Para"x"}` all succeed. Typing these parameters as the
---strict `Blocks` list makes every idiomatic constructor call report
---`missing-fields`, so constructor parameters use this alias instead.
---@alias BlocksLike Blocks|Block[]|Block|string

---As `BlocksLike`, for parameters the manual documents as `Inlines`.
---Verified: `pandoc.Para("x")`, `pandoc.Para(pandoc.Str"x")` and
---`pandoc.Para{pandoc.Str"x"}` all succeed against pandoc 3.11.
---@alias InlinesLike Inlines|Inline[]|Inline|string
...
---@field slide_level? integer Force header level of slides. Optional at construction: `pandoc.WriterOptions{}` succeeds against pandoc 3.11.
