...
---Two duplicate `---@field insert` lines made the language server resolve only
---the first, so the documented `list:insert(value)` form reported a missing
---argument. Both forms work against pandoc 3.11; a union expresses that.
---@field insert fun(self: List<`T`>, pos_or_value: integer|`T`, value?: `T`) `list:insert(value)` appends; `list:insert(pos, value)` inserts at `pos`. Both forms work against pandoc 3.11. Declared as one permissive signature because two duplicate `---@field insert` lines made the language server resolve only the first, so the append form reported a missing argument.
