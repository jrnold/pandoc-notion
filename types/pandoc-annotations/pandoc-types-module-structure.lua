...
---@field make_sections fun(blocks: BlocksLike, opts?: MakeSectionsOptions): Blocks Puts `Blocks` into a hierarchical structure: a list of sections (each a `Div` with class "section" and first element a `Header`).
---@field slide_level fun(blocks: BlocksLike|Pandoc): integer Find level of header that starts slides (defined as the least header level that occurs before a non-header/non-hrule in the blocks).
...
---@field table_of_contents fun(toc_source: BlocksLike|Pandoc|ChunkedDoc, opts?: WriterOptions): BulletList Generates a table of contents for the given object.
